...
assert_present win.html "var EDGE_LIMIT = 0.88;"
assert_present win.html "(Math.random() * 2 - 1) * (STEP / 2) * EDGE_LIMIT"
assert_absent win.html "EDGE_MIN"
...
# The page reads as a real checkout, so the card fields must not invite the
# browser to autofill a saved card into a page that goes nowhere.
assert_absent checkout.html "autocomplete=\"cc-number\""
assert_absent checkout.html "autocomplete=\"cc-exp\""
assert_absent checkout.html "autocomplete=\"cc-csc\""
